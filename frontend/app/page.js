...
export default function LandingPage() {
  ...
  // Why Cards
  const whyCards = [
    { title: 'Instant Digital Visitor Pass',          desc: 'Visitors receive secure virtual passes via Email/WhatsApp eliminating manual paper slips.',                                     className: 'card1', icon: 'M12 2a5 5 0 0 0-5 5v4H6v10h12V11h-1v-4a5 5 0 0 0-5-5z' },
    { title: 'Powerful Live Dashboard',               desc: 'Track check-ins, check-outs, approvals and analytics in real time.',                                                            className: 'card2', icon: 'M12 2a5 5 0 0 0-5 5v4H6v10h12V11h-1v-4a5 5 0 0 0-5-5z' },
    { title: 'Conference Room Booking + Email Alerts',desc: 'Employees can instantly book conference rooms. Organizers receive confirmation notifications.',                                  className: 'card3', icon: 'M12 2a5 5 0 0 0-5 5v4H6v10h12V11h-1v-4a5 5 0 0 0-5-5z' },
    { title: 'Company Specific Public URL',           desc: 'Each company receives a dedicated access link where employees log in via OTP. No HR dependency, no onboarding workload.',      className: 'card4', icon: 'M12 2a5 5 0 0 0-5 5v4H6v10h12V11h-1v-4a5 5 0 0 0-5-5z' },
    { title: 'Zero Manual Work',                      desc: 'No registers, no spreadsheets, no paper passes. Everything automated.',                                                         className: 'card5', icon: 'M12 2a5 5 0 0 0-5 5v4H6v10h12V11h-1v-4a5 5 0 0 0-5-5z' },
    { title: 'Secure & Enterprise Ready',             desc: 'Role based authentication, encryption and compliance aligned security.',                                                        className: 'card6', icon: 'M12 2a5 5 0 0 0-5 5v4H6v10h12V11h-1v-4a5 5 0 0 0-5-5z' },
  ];

  // Steps
  const steps = [
    { title: 'Visitor Registration', desc: 'Visitors can quickly register their arrival using either by scanning a QR code or a web link, making the check-in process seamless.', color: 'orange', icon: 'M12 12a5 5 0 1 0-5-5 5 5 0 0 0 5 5Zm0 2c-4.42 0-8 2.24-8 5v1h16v-1c0-2.76-3.58-5-8-5Z' },
    { title: 'Visitor Notification',  desc: 'Visitor receives instant notification via email or WhatsApp.',                                                                         color: 'purple', icon: 'M12 12a5 5 0 1 0-5-5 5 5 0 0 0 5 5Zm0 2c-4.42 0-8 2.24-8 5v1h16v-1c0-2.76-3.58-5-8-5Z' },
    { title: 'Digital Pass Issued',   desc: 'Visitor receives a secure digital pass. Valid for specified duration and location.',                                                   color: 'blue',   icon: 'M12 12a5 5 0 1 0-5-5 5 5 0 0 0 5 5Zm0 2c-4.42 0-8 2.24-8 5v1h16v-1c0-2.76-3.58-5-8-5Z' },
    { title: 'Track & Analyze',       desc: 'Monitor activity on a live dashboard. Generate reports and maintain compliance.',                                                      color: 'pink',   icon: 'M12 12a5 5 0 1 0-5-5 5 5 0 0 0 5 5Zm0 2c-4.42 0-8 2.24-8 5v1h16v-1c0-2.76-3.58-5-8-5Z' },
  ];
  ...
  // Industries
  const industries = [
    { title: 'Corporates',               desc: 'Enterprise offices with high visitor traffic and multiple meeting rooms',              color: 'blue',   icon: 'M6 3h5v18H6zM13 7h5v14h-5z' },
    { title: 'IT Parks',                 desc: 'Technology campuses with multiple tenants and shared conference facilities',           color: 'violet', icon: 'M6 3h5v18H6zM13 7h5v14h-5z' },
    { title: 'Co-working Spaces',        desc: 'Flexible workspaces managing visitors for multiple companies',                        color: 'green',  icon: 'M6 3h5v18H6zM13 7h5v14h-5z' },
    { title: 'Manufacturing Units',      desc: 'Production facilities requiring strict security and visitor tracking',                color: 'amber',  icon: 'M6 3h5v18H6zM13 7h5v14h-5z' },
    { title: 'Enterprises',              desc: 'Large organizations with multi-location visitor management needs',                    color: 'red',    icon: 'M6 3h5v18H6zM13 7h5v14h-5z' },
    { title: 'Educational Institutions', desc: 'Universities and colleges managing campus visitors and event bookings',              color: 'sky',    icon: 'M6 3h5v18H6zM13 7h5v14h-5z' },
  ];

  // Testimonials
  const testimonials = [
    { name: 'Rajesh Kumar', role: 'IT Manager',       company: 'Tech Solutions Pvt Ltd', review: 'Promeet has completely transformed how we manage visitors. The digital passes and real-time dashboard have eliminated all manual work. Highly recommended!',  featured: true,  avatar: '👨‍💼' },
    { name: 'Priya Sharma', role: 'HR Director',      company: 'Global Enterprises',     review: 'The conference room booking feature is a game-changer. No more double bookings or confusion. Our employees love how easy it is to use.',                       featured: false, avatar: '👩‍💼' },
    { name: 'Amit Patel',   role: 'Facility Manager', company: 'Manufacturing Co.',      review: 'Security has improved significantly since we started using Promeet. We always know who is on our premises. WhatsApp notifications are very convenient.',       featured: false, avatar: '👨‍💼' },
  ];
  ...
  // FAQs
  const faqs = [
    { q: 'How does the 15-day trial work?',                       a: `Pay just ${CONFIG.trial.price} to start your ${CONFIG.trial.duration} trial. You get full access to all Professional plan features. After the trial, you can choose to continue with a paid plan or cancel anytime with no further charges.` },
  ...
  const pricingPlans = [
    {
      ...
      features: [`Valid for ${CONFIG.trial.duration}`, `${CONFIG.trial.visitorLimit} Visitor Bookings`, `${CONFIG.trial.conferenceLimit} Conference Bookings`, `${CONFIG.trial.roomLimit} Conference Rooms`],
      cta: 'Get Started',
    ...
    },
    {
      ...
      features: ['Unlimited Visitors', `${CONFIG.pricing.business.conferenceLimit} Conference Bookings/month`, `Up to ${CONFIG.pricing.business.roomLimit} Conference Rooms`, 'Advanced Analytics & Reports'],
      cta: 'Get Started',
      ctaLink: CONFIG.company.loginUrl,
      popular: true,
    },
    {
      ...
      features: ['Unlimited Visitors', 'Unlimited Conference Bookings', 'Unlimited Conference Rooms', 'Customised Support'],
      cta: 'Contact Sales',
  ...
  return (
    <>
      ...
      >
        ...
        <img src="https://upload.wikimedia.org/wikipedia/commons/6/6b/WhatsApp.svg" alt="" aria-hidden="true" className="waIcon" width={44} height={44} />
      ...
      <section className="hero">
        ...
        <p>
          A platform designed to digitalize organization entry management, streamline conference bookings and ensure a professional visitor experience.
        </p>
        ...
        <div className="dashboardCards">
          <div className="dashCard live">
            ...
            <div className="liveStats">
              <div className="stat"><h2>{CONFIG.stats.todayVisitors}</h2><p>Visitors Today</p></div>
              <div className="stat"><h2>{CONFIG.stats.activeMeetings}</h2><p>Active Meetings</p></div>
            </div>
          </div>
            ...
            {steps.map((step, i) => (
              <div key={i} className="stepCard">
                <div className={`icon ${step.color}`}>
                  <svg viewBox="0 0 24 24" fill="white"><path d={step.icon} /></svg>
                </div>
        ...
        <div className="pricingCards">
          {pricingPlans.map((plan, i) => (
            <div key={i} className={`pricingCard ${plan.popular ? 'featured' : ''}`}>
              ...
              <div className="planPrice">{plan.price} {plan.period && <span>/ {plan.period}</span>}</div>
              <p className="planDuration">{plan.duration}</p>
              <ul className="planFeatures">
                {plan.features.map((feature, j) => <li key={j}>{feature}</li>)}
              </ul>
              <Link href={plan.ctaLink} className="planCta">{plan.cta}</Link>
            </div>
          ))}
        </div>
      </section>
            ...
            {securityFeatures.map((item, i) => (
              <div key={i} className="securityCard">
                <div className="securityIcon" style={{ background: item.color }}>
                  <svg viewBox="0 0 24 24" fill="white"><path d={item.icon} /></svg>
                </div>
                <h3>{item.title}</h3>
                <p>{item.desc}</p>
              </div>
      ...
      <footer className="footer">
        <div className="footerTop">
          <div className="footerBrand fadeUp">
            ...
            <div className="footerCta">
              <Link href={CONFIG.company.whatsapp} target="_blank" rel="noopener noreferrer" className="btnPrimary">WhatsApp →</Link>
              <Link href={`mailto:${CONFIG.company.email}`} className="btnSecondary">Email →</Link>
            </div>
          </div>

          <div className="footerLinks fadeUp delay1">
            <h4>Key Features</h4>
            <ul>{footerFeatures.map((f, i) => <li key={i}>{f}</li>)}</ul>
          </div>

          <div className="footerLinks fadeUp delay2">
            <h4>Industries</h4>
            <ul>{industries.map((ind, i) => <li key={i}>{ind.title}</li>)}</ul>
          </div>
        </div>
        ...
        <div className="footerBottom fadeUp delay4">
          ...
          <div className="footerLinksInline">
            <Link href="https://zodopt.com/privacy-policy/" target="_blank" rel="noopener noreferrer">Privacy Policy</Link>
            <Link href="https://zodopt.com/terms-and-conditions/" target="_blank" rel="noopener noreferrer">Terms and Conditions</Link>
          </div>
        </div>
      </footer>
    </>
